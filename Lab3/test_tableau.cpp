...
bool test5()
{
    ...
    if (t2.taille() != 100) {
        std::cout << t2.taille() << " ==> ECHEC t2.taille()" << std::endl;
        return false;
    }
...
int main()
{
    bool testsreussis = true // Cette ligne est uniquement utile pour mettre la ligne suivante en commentaire sans enlever le && de la 2e ligne suivante!
        && test1()
        && test2()
        && test3()
        && test4()
    ...
    if (testsreussis) {
        ...
        std::cout << "=> Oups, au moins un test n'a pas été réussi :( ." << std::endl;
