...
class Element
{
  ...
  public:
    Element(string _identificateur)
        : identificateur(_identificateur), rang(0), suivant(nullptr)
    {
    }
    ...
    string representantCommun(Element *el2)
    {
        ...
        Element *el2_ptr = el2;
        int r1 = el1_ptr->rang, r2 = el2_ptr->rang;
        string rep = "Pas d'identificateur en commun.";

        // On parcourt les deux listes de representants simultanément pour éviter le recours à une liste
        while (el1_ptr != nullptr && el2_ptr != nullptr)
        {
            if (el1_ptr == el2_ptr)
            {
                rep = el1_ptr->identificateur;
                break;
            }
            else if (r1 < r2)
            {
...
// Gérer les 3 dernieres requetes
...
// Gérer les entrées et formater les commandes / requetes
...
int main(int argc, char const *argv[])
{
    ...
    while (getline(cin, input))
    {
        ...
        if (count(input.begin(), input.end(), '.') > 1 || count(input.begin(), input.end(), '?') > 1)
        {
            cerr << "Entrees non conforme." << endl;
            exit(-1);
        }
